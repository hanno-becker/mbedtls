...
int main( int argc, char *argv[] )
{
    ...
    if( ( return_val = mbedtls_rsa_validate( &N, &P, &Q, &D, &E,
                                      NULL, NULL ) ) != 0 )
    {
        mbedtls_printf( " failed\n  ! mbedtls_rsa_validate returned %d\n\n",
                        return_val );
        goto exit;
    }

    if( ( return_val = mbedtls_rsa_validate_crt( &P, &Q, &D,
                                                 &DP, &DQ, &QP ) ) != 0 )
    {
        mbedtls_printf( " failed\n  ! mbedtls_rsa_validate_crt returned %d\n\n",
                        return_val );
        goto exit;
    }

...
static int x509_crt_get_ext( unsigned char **p,
                             unsigned char *end,
                             mbedtls_x509_crt_frame *frame )
{
    int ret;
...
#endif
    {
        ret = x509_crt_get_ext( &p, end, frame );
        if( ret != 0 )
            return( ret );
    }
...
find_parent:
...

        {
            ...
            ret = x509_crt_find_parent( &child_sig, child_crt->next, trust_ca,
                                        &parent_crt, &parent_is_trusted,
                                        &signature_is_good,

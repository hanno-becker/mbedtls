...
#define SSL_PROCESS_SERVER_HELLO_DONE   4
    ...
    while( ( state = mbedtls_mps_reader_state( reader ) ) !=
           SSL_PROCESS_SERVER_HELLO_DONE )
    {
        if( state == SSL_PROCESS_SERVER_HELLO_HEADER )
        {
             ...
             * returned by mbedtls_mps_reader_get */
             ...
             */
            mbedtls_mps_reader_get( reader, 35, &header, NULL );
             ...
             * 37+n . 37+n  compression_method
             */
            mbedtls_mps_reader_get( reader, session_id_len + 3, &buf, NULL );
            ...
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "server hello, chosen ciphersuite: %s",
                                        ssl->handshake->ciphersuite_info->name ) );

            /* Commit what we have parsed so far */
            mbedtls_mps_reader_commit( reader, -1u, SSL_PROCESS_SERVER_HELLO_EXTENSIONS_START );
        }
        else if( state == SSL_PROCESS_SERVER_HELLO_EXTENSIONS_START )
        {
             ...
             * 38+n . 39+n  extensions length (optional)
             */
            mbedtls_mps_reader_get( reader, 2, &ext_header, NULL );
            ...
            mbedtls_mps_reader_group_open( reader, 2 + ext_len );

            mbedtls_mps_reader_commit( reader, -1u, SSL_PROCESS_SERVER_HELLO_EXTENSIONS_PARSE );
        }
        else if( state == SSL_PROCESS_SERVER_HELLO_EXTENSIONS_PARSE )
        {
            ...
            if( mbedtls_mps_reader_bytes_remaining( reader ) == 0 )
            {
                mbedtls_mps_reader_commit( reader, -1u, SSL_PROCESS_SERVER_HELLO_EXTENSIONS_END );
                continue;
            }
            ...
            mbedtls_mps_reader_get( reader, 4, &hdr, NULL );

            ext_id  = ( ( hdr[0] <<  8 ) | ( hdr[1] ) );
            ext_len = ( ( hdr[2] <<  8 ) | ( hdr[3] ) );

            ret = mbedtls_mps_reader_get( reader, ext_len, &ext, NULL );
            if( ret == MBEDTLS_ERR_MPS_READER_FRAGMENT_TOO_SMALL )
            {
                /* Stash the last two reads and abort */
                /* TODO: Think whether we need this as an explicit call. */
                mbedtls_mps_reader_stash( reader, 2 );
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
        ...
        else if( state == SSL_PROCESS_SERVER_HELLO_EXTENSIONS_END )
        {
            ...
            mbedtls_mps_reader_group_close( reader );

            mbedtls_mps_reader_commit( reader, -1u, SSL_PROCESS_SERVER_HELLO_DONE );
        }
        else
        {
            /* Should never happen */
                                           ...
                                           mbedtls_mps_handshake_in *msg )
{
     ...
     *   ProtocolVersion server_version;
     */
    mbedtls_mps_reader_get( reader, 3, &p, NULL );
    ...
    mbedtls_mps_reader_get( reader, cookie_len, &p, NULL );
    MBEDTLS_SSL_DEBUG_BUF( 3, "cookie", p, cookie_len );

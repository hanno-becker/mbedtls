    ...
    if( mbedtls_ssl_ciphersuite_uses_psk( ciphersuite_info ) )
    {
        ...
        if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_RSA_PSK )
        {
            if( ( ret = ssl_rsa_encrypt_partial_pms(
                     ssl, ssl->handshake->premaster + MBEDTLS_RSA_PMS_LEN_BYTES,
                     MBEDTLS_RSA_PMS_LEN, p, end - p, &n ) ) != 0 )
            {
                return( ret );
            }
            p += n;
...
/*         i = 4; */
/*         n = ssl->conf->psk_identity_len; */
/*         if( i + 2 + n > MBEDTLS_SSL_MAX_OUT_CONTENT_LEN ) */
/*         { */
/*             MBEDTLS_SSL_DEBUG_MSG( 1, ( "psk identity too long or " */
/*                                         "SSL buffer too short" ) ); */
/*             return( MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL ); */
...
/* #endif */
        ...
        if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_PSK )
        {
             ...
             */
            /* ret = mbedtls_ecdh_make_public( &ssl->handshake->ecdh_ctx, &n, */
            /*         &ssl->out_msg[i], MBEDTLS_SSL_MAX_OUT_CONTENT_LEN - i, */
            /*         ssl->conf->f_rng, ssl->conf->p_rng ); */
            /* if( ret != 0 ) */
            /* { */
            /*     MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ecdh_make_public", ret ); */
    ...
    if( ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE )
    {
        /* i = 4; */
        /* ret = mbedtls_ecjpake_write_round_two( &ssl->handshake->ecjpake_ctx, */
        /*         ssl->out_msg + i, MBEDTLS_SSL_MAX_OUT_CONTENT_LEN - i, &n, */
        /*         ssl->conf->f_rng, ssl->conf->p_rng ); */
        /* if( ret != 0 ) */
        /* { */
        /*     MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ecjpake_write_round_two", ret ); */

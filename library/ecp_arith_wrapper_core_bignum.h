...
#define ECP_MPI_REDUCE(X)                                                     \
    MBEDTLS_MPI_CHK( mbedtls_mpi_core_mod_reduce(                             \
                         *(X), *(X), grp->P, grp->RP ) )
...
#define ECP_MPI_DOUBLE( X )                                                    \
    ECP_MPI_ADD( (X), (X), (X) )
#define ECP_MPI_REDUCE(X)                                                      \
    MBEDTLS_MPI_CHK( mbedtls_mpi_core_mod_reduce_p(                            \
                         (X), (X), &grp->P, &grp->RP ) )
#define ECP_MPI_SET1( X )                                                      \
    do {                                                                       \
        ECP_MPI_ZERO( X );                                                     \
...
#elif defined(ECP_ARITH_WRAPPER_CORE_BIGNUM_BUFS_BY_REF)
    MBEDTLS_MPI_CHK( mbedtls_mpi_core_montmul_p( x, x, &one_buf, &grp->P, &grp->T, grp->mm  ) );
#endif
...
#elif defined(ECP_ARITH_WRAPPER_CORE_BIGNUM_BUFS_BY_REF)
    MBEDTLS_MPI_CHK( mbedtls_mpi_core_montmul_p( x, x, &grp->RP, &grp->P, &grp->T, grp->mm  ) );
#endif
cleanup:
    return( ret );
}
...
{
    memcpy( x->p, raw, ciL * getGrp(grp)->P.n );
}
    ...
    mbedtls_ecp_group *src )
{
    ...
    ecp_group_internal_selftest( grp );

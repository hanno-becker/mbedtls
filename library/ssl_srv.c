                                               ...
                                               size_t len )
{
    ...
    for( p = buf + 2; p < end; p += 2 ) {
        ...
        {
            mbedtls_sig_hash_set_add( &ssl->handshake->hash_algs, sig_cur, md_cur );
                                  ...
                                  const mbedtls_ssl_ciphersuite_t **ciphersuite_info )
{
    ...
    if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3                                       &&
        ( sig_type = mbedtls_ssl_get_ciphersuite_sig_alg( suite_info ) ) != MBEDTLS_PK_NONE &&
        mbedtls_sig_hash_set_find( &ssl->handshake->hash_algs, sig_type ) == MBEDTLS_MD_NONE )
    {
...
read_record_header:
    ...
    if( sig_hash_alg_ext_present == 0 )
    {
        ...
        mbedtls_sig_hash_set_const_hash( &ssl->handshake->hash_algs, md_default );
    }
...
have_ciphersuite:
    ...
    if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
    {
        ...
        {
            mbedtls_md_type_t md_alg = mbedtls_sig_hash_set_find( &ssl->handshake->hash_algs,
                                                                  sig_alg );
...
curve_matching_done:
    ...
    if( mbedtls_ssl_ciphersuite_uses_server_signature( ciphersuite_info ) )
    {
        ...
        if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
        {
            ...
            if( sig_alg == MBEDTLS_PK_NONE ||
                ( md_alg = mbedtls_sig_hash_set_find( &ssl->handshake->hash_algs, sig_alg ) ) == MBEDTLS_MD_NONE )
            {
                MBEDTLS_SSL_DEBUG_MSG( 1, ( "should never happen" ) );

                                  ...
                                  const mbedtls_ssl_ciphersuite_t **ciphersuite_info )
{
    ...
    if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3                     &&
        ( sig_type = mbedtls_ssl_get_ciphersuite_sig_alg( suite_info ) )
          != MBEDTLS_PK_NONE                                              &&
        mbedtls_sig_hash_set_find( &ssl->handshake->hash_algs, sig_type )
          == MBEDTLS_MD_NONE )
    {
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "ciphersuite mismatch: no suitable hash " 
                                    "algorithm for signature algorithm %d",
                                    sig_type ) );
        return( 0 );
    }
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 &&
...
curve_matching_done:
    ...
    if( mbedtls_ssl_ciphersuite_uses_server_signature( ciphersuite_info ) )
    {
        ...
        if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
        {
            ...
            if( sig_alg == MBEDTLS_PK_NONE ||
                ( md_alg = mbedtls_sig_hash_set_find( &ssl->handshake->hash_algs,

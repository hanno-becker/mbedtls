...
static int ssl_process_client_hello( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = mbedtls_ssl_parse_hs_msg( ssl, MBEDTLS_SSL_HS_CLIENT_HELLO,
                                          ssl_client_hello_v3_parse ) ) != 0 )
    {
        return( ret );
    }

    /* MBEDTLS_SSL_PROC_CHK( mbedtls_mps_read( &ssl->messaging.mps ) ); */
    ...
    /* MBEDTLS_SSL_PROC_CHK( mbedtls_mps_read_handshake( &ssl->messaging.mps, */
    /*                                                   &msg ) ); */
    ...
    /* MBEDTLS_SSL_PROC_CHK( ssl_client_hello_v3_parse( ssl, buf, */
    /*                                                  msg.length ) ); */

    /* MBEDTLS_SSL_PROC_CHK( mbedtls_reader_commit_ext( msg.handle ) ); */
    /* MBEDTLS_SSL_PROC_CHK( mbedtls_mps_read_consume( &ssl->messaging.mps ) ); */

/*    MBEDTLS_SSL_PROC_CHK( ssl_client_hello_fetch( ssl, &buf, &buflen ) ); */
/*     if( ret == SSL_CLIENT_HELLO_V3 ) */
/*         MBEDTLS_SSL_PROC_CHK( ssl_client_hello_v3_parse( ssl, buf, buflen ) ); */
/* #if defined(MBEDTLS_SSL_SRV_SUPPORT_SSLV2_CLIENT_HELLO) */
/*     else */
/*         MBEDTLS_SSL_PROC_CHK( ssl_client_hello_v2_parse( ssl, buf, buflen ) ); */
/* #endif /\* MBEDTLS_SSL_SRV_SUPPORT_SSLV2_CLIENT_HELLO *\/ */
...
/*     /\* Distinguish between SSLv2 and SSLv3+higher *\/ */
/* #if defined(MBEDTLS_SSL_SRV_SUPPORT_SSLV2_CLIENT_HELLO) */
/* #if defined(MBEDTLS_SSL_PROTO_DTLS) */
                                      ...
                                      size_t buflen )
{
     ...
     *   ..  .  ..   challenge
     */
    MBEDTLS_SSL_DEBUG_BUF( 4, "record contents", buf, buflen );
    ...
    if( buflen != 6 + ciph_len + sess_len + chal_len )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad client hello message" ) );
        return( MBEDTLS_ERR_SSL_BAD_HS_CLIENT_HELLO );
    }
    ...
    for( i = 0, p = buf + 6; i < ciph_len; i += 3, p += 3 )
    {
            ...
            p[2] == (unsigned char)( ( MBEDTLS_SSL_FALLBACK_SCSV_VALUE      ) & 0xff ) )
        {
            ...
            if( ssl->minor_ver < ssl->conf->max_minor_ver )
            {
                ...
                ssl->send_alert = MBEDTLS_SSL_ALERT_LEVEL_FATAL;
                ssl->alert_type = MBEDTLS_SSL_ALERT_MSG_INAPROPRIATE_FALLBACK;
                return( MBEDTLS_ERR_SSL_BAD_HS_CLIENT_HELLO );
            }
                                      ...
                                      size_t buflen )
{
    ...
    size_t ciph_len, sess_len, comp_len, ext_len;
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    ...
    if( sess_len > sizeof( ssl->session_negotiate->id ) ||
        sess_len + 34 + 2 > buflen ) /* 2 for cipherlist length field */
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad client hello message" ) );
        ssl->send_alert = MBEDTLS_SSL_ALERT_LEVEL_FATAL;
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        if( cookie_offset + 1 + cookie_len + 2 > buflen )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad client hello message" ) );
            ssl->send_alert = MBEDTLS_SSL_ALERT_LEVEL_FATAL;
            ssl->alert_type = MBEDTLS_SSL_ALERT_MSG_PROTOCOL_VERSION;
    ...
    if( ciph_len < 2 ||
        ciph_len + 2 + ciph_offset + 1 > buflen || /* 1 for comp. alg. len */
        ( ciph_len % 2 ) != 0 )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad client hello message" ) );
        ssl->send_alert = MBEDTLS_SSL_ALERT_LEVEL_FATAL;
    ...
    if( ( ssl->major_ver != 3 ) || ( ssl->minor_ver != 0 ) )
    {
        ...
        if( buflen > ext_offset )
        {
            if( buflen < ext_offset + 2 )
            {
                MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad client hello message" ) );
                ssl->send_alert = MBEDTLS_SSL_ALERT_LEVEL_FATAL;
                ssl->alert_type = MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR;
...
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
static void ssl_write_encrypt_then_mac_ext( mbedtls_ssl_context const *ssl,
                                            unsigned char *buf,
                                            unsigned char *end,
                                            size_t *olen )
{
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
static void ssl_write_extended_ms_ext( mbedtls_ssl_context const *ssl,
                                       unsigned char *buf,
                                       unsigned char *end,
                                       size_t *olen )
{
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
static void ssl_write_session_ticket_ext( mbedtls_ssl_context const *ssl,
                                          unsigned char *buf,
                                          unsigned char *end,
                                          size_t *olen )
{
...
static void ssl_write_renegotiation_ext( mbedtls_ssl_context const *ssl,
                                         unsigned char *buf,
                                         unsigned char *end,
                                         size_t *olen )
{
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
static void ssl_write_max_fragment_length_ext( mbedtls_ssl_context const *ssl,
                                               unsigned char *buf,
                                               unsigned char *end,
                                               size_t *olen )
{
...
#if defined(MBEDTLS_ECDH_C) || defined(MBEDTLS_ECDSA_C) || \
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
static void ssl_write_supported_point_formats_ext( mbedtls_ssl_context const *ssl,
                                                   unsigned char *buf,
                                                   unsigned char *end,
                                                   size_t *olen )
{
...
#if defined(MBEDTLS_SSL_ALPN )
static void ssl_write_alpn_ext( mbedtls_ssl_context const *ssl,
                                unsigned char *buf,
                                unsigned char *end,
                                size_t *olen )
{
    size_t alpn_chosen_len;
    if( ssl->alpn_chosen == NULL )
    {
        *olen = 0;
        return;
    }
    alpn_chosen_len = strlen( ssl->alpn_chosen );

    if( end                       < buf            ||
        end - buf                 < 4              ||
        (size_t)( end - buf ) - 4 < alpn_chosen_len )
    {
        *olen = 0;
        return;
    }
...
static int ssl_process_server_hello_preprocess ( mbedtls_ssl_context *ssl );
static int ssl_process_server_hello_postprocess( mbedtls_ssl_context *ssl );
static int ssl_process_server_hello_write      ( mbedtls_ssl_context *ssl,
                                                 unsigned char* buf,
                                                 size_t buflen,
                                                 size_t *olen );

#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
/* Orchestrating function for HelloVerifyRequest; called from ssl_process_server_hello */
static int ssl_process_hello_verify( mbedtls_ssl_context *ssl );

/* HelloVerifyRequest handling sub-routines */
static int ssl_process_hello_verify_write( mbedtls_ssl_context const *ssl,
                                           unsigned char* buf,
                                           size_t buflen,
                                           size_t *olen );
#endif /* MBEDTLS_SSL_DTLS_HELLO_VERIFY */
...
static int ssl_process_server_hello( mbedtls_ssl_context *ssl )
{
...
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
    /* Send HelloVerifyRequest instead if we haven't received a valid cookie */
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM &&
        ssl->handshake->verify_cookie_len != 0 )
    {
     ...
     * multiple times. */
    MBEDTLS_SSL_PROC_CHK( ssl_process_server_hello_preprocess( ssl ) );
    ...
    MBEDTLS_SSL_PROC_CHK( mbedtls_ssl_write_hs_msg( ssl,
                                          MBEDTLS_SSL_HS_SERVER_HELLO,
                                          MBEDTLS_MPS_LENGTH_UNKNOWN,
                                          ssl_process_server_hello_write ) );

    /* MBEDTLS_SSL_PROC_CHK( ssl_process_server_hello_write( ssl, ssl->out_msg, */
...
static int ssl_process_hello_verify( mbedtls_ssl_context *ssl )
{
    ...
    /* Writing step */
    MBEDTLS_SSL_PROC_CHK( ssl_process_hello_verify_write( ssl,
                                                  ssl->out_msg,
                                                  MBEDTLS_SSL_MAX_CONTENT_LEN,
                                                  &ssl->out_msglen ) );
    ssl->out_msgtype = MBEDTLS_SSL_MSG_HANDSHAKE;
    ...
    /* NOTE: Same comment on the order of postprocess and dispatch */
                                           ...
                                           size_t *olen )
{
    ...
    /* This is set by ssl_write_record. */
    p += 4;
    ...
    *cookie_len_byte = (unsigned char)( p - ( cookie_len_byte + 1 ) );
    MBEDTLS_SSL_DEBUG_BUF( 3, "cookie", cookie_len_byte + 1, *cookie_len_byte );
...
static int ssl_process_server_hello_preprocess( mbedtls_ssl_context *ssl )
{
    ...
    /* Gather server random data (36 bytes)
     *
     * The first four bytes are extracted from the system time, if available.
     * The remaining 32 or 36 bytes are obtained from the configured PRNG.
     *
     * Q: This should be done much earlier, e.g. at handshake_init.
     */
...
#if defined(MBEDTLS_HAVE_TIME)
    t = mbedtls_time( NULL );
    ssl->handshake->randbytes[32] = (unsigned char)( t >> 24 );
    ssl->handshake->randbytes[33] = (unsigned char)( t >> 16 );
    ssl->handshake->randbytes[34] = (unsigned char)( t >>  8 );
    ssl->handshake->randbytes[35] = (unsigned char)( t       );
    MBEDTLS_SSL_DEBUG_MSG( 3, ( "server hello, current time: %lu", t ) );
#else
    if( ( ret = ssl->conf->f_rng( ssl->conf->p_rng,
                                  ssl->handshake->randbytes + 32, 4 ) ) != 0 )
        return( ret );
#endif /* MBEDTLS_HAVE_TIME */

    if( ( ret = ssl->conf->f_rng( ssl->conf->p_rng,
                                  ssl->handshake->randbytes + 36, 28 ) ) != 0 )
        return( ret );

    /* Check for session resumption
     *
     * Two ways to resume a session:
     * 1. Ticket-based
     *    If present, the client's ticket has already been validated
     *    and potentially been extracted into ssl->session_negotiate
     *    at this stage. In this case, ssl->resume == 1.
     * 2. Session-ID based
     *    Check if we have a valid session cache entry for the ID
     ...
     *   from a ticket here, it overwrites whatever was negotiated in the
     *   ClientHello.
     */
    ...
    if( ssl->handshake->resume == 0 &&
        ...
        ssl->conf->f_get_cache( ssl->conf->p_cache,
                                           ...
                                           size_t *olen )
{
    ...
    if( buflen < 42 + ssl->session_negotiate->id_len )
    {
        return( MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL );
    }
...
/*    p += 4; */
    ...
    /*     6  .   9   UNIX time()
     *    10  .  37   random bytes               */
    memcpy( p, ssl->handshake->randbytes + 32, 32 );
    p += 32;
    ...
    *p++ = (unsigned char) ssl->session_negotiate->id_len;
    ...
    MBEDTLS_SSL_DEBUG_MSG( 3, ( "server hello, session id len.: %d",
                                ssl->session_negotiate->id_len ) );
    MBEDTLS_SSL_DEBUG_BUF( 3,   "server hello, session id", buf + 39,
                           ssl->session_negotiate->id_len );
    ...
    *p++ = (unsigned char)( ssl->session_negotiate->compression      );
    ...
    if( ( ssl->major_ver != 3 ) || ( ssl->minor_ver != 0 ) )
    {
         ...
         */
        ssl_write_renegotiation_ext( ssl, p + 2 + ext_len, end, &ext_len );
        ext_len_total += ext_len;

#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
        ssl_write_max_fragment_length_ext( ssl, p + 2 + ext_len, end, &ext_len );
        ext_len_total += ext_len;
#endif

#if defined(MBEDTLS_SSL_TRUNCATED_HMAC)
        ssl_write_truncated_hmac_ext( ssl, p + 2 + ext_len_total, end, &ext_len );
        ext_len_total += ext_len;
#endif

#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
        ssl_write_encrypt_then_mac_ext( ssl, p + 2 + ext_len_total, end, &ext_len );
        ext_len_total += ext_len;
#endif

#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
        ssl_write_extended_ms_ext( ssl, p + 2 + ext_len_total, end, &ext_len );
        ext_len_total += ext_len;
#endif

#if defined(MBEDTLS_SSL_SESSION_TICKETS)
        ssl_write_session_ticket_ext( ssl, p + 2 + ext_len_total, end, &ext_len );
        ext_len_total += ext_len;
#endif

#if defined(MBEDTLS_ECDH_C) || defined(MBEDTLS_ECDSA_C) ||      \
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
        ssl_write_supported_point_formats_ext( ssl, p + 2 + ext_len_total, end, &ext_len );
        ext_len_total += ext_len;
#endif
...
#if defined(MBEDTLS_SSL_ALPN)
        ssl_write_alpn_ext( ssl, p + 2 + ext_len_total, end, &ext_len );
        ext_len_total += ext_len;
#endif

        MBEDTLS_SSL_DEBUG_MSG( 3, ( "server hello, total extension length: %d", ext_len_total ) );

        if( ext_len_total > 0 )
        {
            *p++ = (unsigned char)( ( ext_len_total >> 8 ) & 0xFF );
            *p++ = (unsigned char)( ( ext_len_total      ) & 0xFF );
            p += ext_len_total;
        }
...
static int ssl_process_server_key_exchange( mbedtls_ssl_context *ssl )
{
    ...
    if( ret == SSL_SERVER_KEY_EXCHANGE_SEND )
    {
        ...
        MBEDTLS_SSL_PROC_CHK( mbedtls_ssl_write_hs_msg( ssl,
                                  ...
                                  MBEDTLS_MPS_LENGTH_UNKNOWN,
                                  ssl_server_key_exchange_write ) );

        /* Prepare CertificateVerify message in output buffer. */
        /* MBEDTLS_SSL_PROC_CHK( ssl_server_key_exchange_write( ssl, ssl->out_msg, */
        /*                                            MBEDTLS_SSL_MAX_CONTENT_LEN, */
        /*                                            &ssl->out_msglen ) ); */
...
static int ssl_server_key_exchange_prepare( mbedtls_ssl_context *ssl )
{
    ...
    /* Pick DH prime modulus for DHE ciphersuites. */
...
#endif /* MBEDTLS_KEY_EXCHANGE__SOME_DHE_ENABLED */

    /* Pick curve to use for ECDHE ciphersuites. */
...
static int ssl_server_key_exchange_coordinate( mbedtls_ssl_context *ssl )
{
    ...
    {
        return( SSL_SERVER_KEY_EXCHANGE_SKIP );
    }
#endif /* MBEDTLS_KEY_EXCHANGE__NON_PFS__ENABLED */

    return( SSL_SERVER_KEY_EXCHANGE_SEND );
}
                                          ...
                                          size_t *olen )
{
    ...
    if( mbedtls_ssl_ciphersuite_uses_ecdhe( ciphersuite_info ) )
    {
        ...
        if( ( ret = mbedtls_ecdh_make_params( &ssl->handshake->ecdh_ctx, &len,
                      p, end - p, ssl->conf->f_rng, ssl->conf->p_rng ) ) != 0 )
        {
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        ...
        if( ( ret = mbedtls_pk_sign( mbedtls_ssl_own_key( ssl ), md_alg,
                        hash, hashlen, p + 2 , &signature_len,
...
static int ssl_process_server_hello_done( mbedtls_ssl_context *ssl )
{
    ...
    MBEDTLS_SSL_PROC_CHK( mbedtls_ssl_write_hs_msg( ssl,
                                  ...
                                  0, ssl_server_hello_done_write ) );

    /* MBEDTLS_SSL_PROC_CHK( ssl_server_hello_done_write( ssl, ssl->out_msg, */
    /*                                     MBEDTLS_SSL_MAX_CONTENT_LEN, */
    /*                                     &ssl->out_msglen ) ); */
    /* ssl->out_msgtype = MBEDTLS_SSL_MSG_HANDSHAKE; */
     ...
     *       to be updated in order to not enter
     *       this function again on retry. */
    MBEDTLS_SSL_PROC_CHK( ssl_server_hello_done_postprocess( ssl ) );
...
cleanup:
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= write server hello done, ret %d", ret ) );
    return( ret );
}
...
static int ssl_server_hello_done_postprocess( mbedtls_ssl_context *ssl )
{
    MBEDTLS_SSL_DEBUG_MSG( 3, ( "ServerHelloDone postprocessing: update state" ) );
...
static int ssl_process_client_key_exchange( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = mbedtls_ssl_parse_hs_msg( ssl, MBEDTLS_SSL_HS_CLIENT_KEY_EXCHANGE,
                                          ssl_client_key_exchange_parse ) ) != 0 )
    {
        return( ret );
    }
    ...
    /* if( msg.type != MBEDTLS_SSL_HS_CLIENT_KEY_EXCHANGE ) */
    /* { */
    /*     ret = MBEDTLS_ERR_SSL_UNEXPECTED_MESSAGE; */
    /*     goto cleanup; */
    /* } */
    ...
    /* MBEDTLS_SSL_PROC_CHK( ssl_client_key_exchange_parse( ssl, buf, */
    /*                                                      msg.length ) ); */
    ...
    /* MBEDTLS_SSL_PROC_CHK( ssl_client_key_exchange_parse( ssl, ssl->in_msg, */
    /*                                              ssl->in_hslen ) ); */
...
static int ssl_process_certificate_verify( mbedtls_ssl_context *ssl )
{
    ...
    if( ret == SSL_CERTIFICATE_VERIFY_READ )
    {
        ...
        if( ( ret = mbedtls_ssl_parse_hs_msg( ssl, MBEDTLS_SSL_HS_CERTIFICATE_VERIFY,
                                           ssl_certificate_verify_parse ) ) != 0 )
        {
            return( ret );
        }
        ...
        /* MBEDTLS_SSL_PROC_CHK( ssl_certificate_verify_parse( ssl, ssl->in_msg, */
        /*                                             ssl->in_hslen ) ); */
...
static int ssl_process_new_session_ticket( mbedtls_ssl_context *ssl )
{
    ...
    MBEDTLS_SSL_PROC_CHK( mbedtls_ssl_write_hs_msg( ssl,
                                  ...
                                  MBEDTLS_MPS_LENGTH_UNKNOWN,
                                  ssl_new_session_ticket_write ) );

    /* Prepare the session ticket message */
    /* MBEDTLS_SSL_PROC_CHK( ssl_new_session_ticket_write( ssl, ssl->out_msg, */
    /*                                             MBEDTLS_SSL_MAX_CONTENT_LEN, */
    /*                                             &ssl->out_msglen ) ); */
                                         ...
                                         size_t *olen )
{
    ...
    buf[0] = ( lifetime >> 24 ) & 0xFF;
    buf[1] = ( lifetime >> 16 ) & 0xFF;
    buf[2] = ( lifetime >>  8 ) & 0xFF;
    buf[3] = ( lifetime       ) & 0xFF;

    buf[4] = (unsigned char)( ( tlen >> 8 ) & 0xFF );
    buf[5] = (unsigned char)( ( tlen      ) & 0xFF );

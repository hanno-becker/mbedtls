...
static inline mbedtls_mpi_uint * mbedtls_ecp_group_get_Z(
    mbedtls_ecp_group *grp )
{
    return( (mbedtls_mpi_uint*) grp->Z );
}
...
static int ecp_double_jac( mbedtls_ecp_group_internal *grp,
                           ...
                           mbedtls_ecp_mpi_internal tmp[4] )
{
#if defined(MBEDTLS_SELF_TEST)
    dbl_count++;
#endif
...
static int ecp_add_mixed_core( mbedtls_ecp_group_internal *grp,
                               mbedtls_ecp_point_internal *R,
                               const mbedtls_ecp_point_internal *P,
                                   ...
                                   const mbedtls_ecp_point_internal *Q )
{
    ...
    ECP_ARITH_START();
    MBEDTLS_MPI_CHK( ecp_add_mixed_core( grp, R, P, Q, tmp ) );
...
add:
    ...
    for( i = 1; i < T_size; i <<= 1 )
    {
        j = i;
        while( j-- )
            MBEDTLS_MPI_CHK( ecp_add_mixed_core( grp, &T[i + j], &T[j], &T[i], tmp ) );
    }
                              ...
                              mbedtls_ecp_restart_ctx *rs_ctx )
{
    ...
    while( i != 0 )
    {
        ...
        MBEDTLS_MPI_CHK( ecp_add_mixed_core( grp, R, R, Txi, tmp ) );
    }
                         ...
                         mbedtls_ecp_restart_ctx *rs_ctx )
{
    ...
    {
        /* Allocate space for T_size'd array of mbedtls_ecp_point_internal,
         * followed by the raw data for their X,Y coordinates.
         *
         * The Z coordinates are allocated separately because they are freed
         * once T is fully computed and normalized. */

        mbedtls_mpi_uint * XY_data;

        size_t const num_limbs = getGrp(grp)->P.n;

        size_t const bytes_per_coord = sizeof(mbedtls_mpi_uint) * num_limbs;
        size_t const bytes_per_ecp = sizeof(mbedtls_ecp_point_internal);
        size_t const bytes_per_xy  = 2 * bytes_per_coord;
        size_t const bytes_per_z   = 1 * bytes_per_coord;

        T      = mbedtls_calloc( T_size, bytes_per_ecp + bytes_per_xy );
        Z_data = mbedtls_calloc( T_size, bytes_per_z );
        if( Z_data == NULL || T == NULL )
        {
            ret = MBEDTLS_ERR_ECP_ALLOC_FAILED;
            goto cleanup;
        }

        XY_data = (mbedtls_mpi_uint*)( T + T_size );

        mbedtls_mpi_uint * cur_X = XY_data;
        mbedtls_mpi_uint * cur_Z = Z_data;
        for( i = 0; i < T_size; i++ )
        {
            mbedtls_mpi_uint * cur_Y = cur_X + num_limbs;

            mbedtls_ecp_point_internal_init( &T[i] );
            MBEDTLS_MPI_CHK( mbedtls_ecp_mpi_internal_from_raw_ref( grp,
                                 getX(&T[i]), cur_X, num_limbs ) );
            MBEDTLS_MPI_CHK( mbedtls_ecp_mpi_internal_from_raw_ref( grp,
                                 getY(&T[i]), cur_Y, num_limbs ) );
            MBEDTLS_MPI_CHK( mbedtls_ecp_mpi_internal_from_raw_ref( grp,
                                 getZ(&T[i]), cur_Z, num_limbs ) );

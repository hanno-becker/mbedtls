        ...
        ) )
    {
        ...
        if( transform->minor_ver == MBEDTLS_SSL_MINOR_VERSION_0 )
        {
            ssl_mac( &transform->md_ctx_enc, transform->mac_enc,
                     data, rec->data_len, rec->ctr, rec->type );
        }
        else
#endif
#if defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1) || \
        ...
        if( transform->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1 )
        {
            ...
            mbedtls_md_hmac_update( &transform->md_ctx_enc,
                                    data, rec->data_len );
            mbedtls_md_hmac_finish( &transform->md_ctx_enc,
                                    data + rec->data_len );
                            ...
                            mbedtls_mps_record *rec )
{
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
        ...
        if( transform->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED )
        {
            ...
            mbedtls_md_hmac_update( &transform->md_ctx_dec,
                                    data, rec->data_len );
            mbedtls_md_hmac_finish( &transform->md_ctx_dec, computed_mac );
            ...
            if( mbedtls_ssl_safer_memcmp( data + rec->data_len, computed_mac,
                                          transform->maclen ) != 0 )
            {
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        if( transform->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0 )
        {
            ...
            volatile size_t real_count = 1;
            volatile unsigned char* padding = data + ( rec->data_len - padlen );

            for( size_t i = 0; i < 256; i++ )
            {
                real_count &= ( i < padlen );
                pad_count += real_count * ( padding[i % padlen] == padlen - 1 );
            }
    ...
    if( auth_done == 0 )
    {
        ...
        memcpy( tmp, data + rec->data_len, transform->maclen );
        ...
        if( transform->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0 )
        {
            ...
            mbedtls_md_hmac_update( &transform->md_ctx_dec, data,
                                    rec->data_len );
            mbedtls_md_hmac_finish( &transform->md_ctx_dec,
                                    data + rec->data_len );
        ...
        MBEDTLS_SSL_DEBUG_BUF( 4, "message  mac", tmp, transform->maclen );
        MBEDTLS_SSL_DEBUG_BUF( 4, "computed mac", data + rec->data_len,
                               transform->maclen );

        if( mbedtls_ssl_safer_memcmp( tmp, data + rec->data_len,
                                      transform->maclen ) != 0 )
        {
#if defined(MBEDTLS_SSL_DEBUG_ALL)
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "message mac does not match" ) );

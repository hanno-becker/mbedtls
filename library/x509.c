...
static int x509_date_is_valid(const x509_time *tm)
{
    int ret = POLARSSL_ERR_X509_INVALID_DATE;

    CHECK_RANGE( 0, 9999, tm->year );
    CHECK_RANGE( 0, 23,   tm->hour );
    CHECK_RANGE( 0, 59,   tm->min  );
    CHECK_RANGE( 0, 59,   tm->sec  );

    switch( tm->mon )
    {
        case 1: case 3: case 5: case 7: case 8: case 10: case 12:
            CHECK_RANGE( 1, 31, tm->day );
            break;
        case 4: case 6: case 9: case 11:
            CHECK_RANGE( 1, 30, tm->day );
            break;
        case 2:
            CHECK_RANGE( 1, 28 + (tm->year % 4 == 0), tm->day );
            break;
        default:
            return( ret );
    }
 ...
 */
static int x509_parse_time( unsigned char **p, size_t len, unsigned int yearlen,
                            x509_time *tm )
{
    int ret;
